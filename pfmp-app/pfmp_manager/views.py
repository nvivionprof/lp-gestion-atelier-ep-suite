...
from .forms import (
    ...
    CompanyImportForm, GeocodeCompaniesForm
)
from .models import (
...
def _geocoding_stats():
    return {
        'total': Company.objects.exclude(status='inactive').count(),
        'avec_coordonnees': Company.objects.exclude(status='inactive').filter(latitude__isnull=False, longitude__isnull=False).count(),
        'a_geocoder': Company.objects.exclude(status='inactive').filter(Q(latitude__isnull=True) | Q(longitude__isnull=True) | Q(geocoding_status='A_GEOCODER') | Q(geocoding_status='')).count(),
        'ok': Company.objects.exclude(status='inactive').filter(geocoding_status='OK').count(),
        'ambigu': Company.objects.exclude(status='inactive').filter(geocoding_status='AMBIGU').count(),
        'echec': Company.objects.exclude(status='inactive').filter(geocoding_status='ECHEC').count(),
        'contacts_a_geocoder': CompanyContact.objects.filter(active=True, use_personal_location_for_student_search=True).filter(Q(personal_latitude__isnull=True) | Q(personal_longitude__isnull=True)).count(),
    }


def geocode_companies(request):
    user, response = _require_login(request)
    if response:
        return response
    if not user.is_prof_like:
        messages.error(request, 'Géocodage réservé aux professeurs ou administrateurs PFMP.')
        return redirect('pfmp_company_list')

    output = ''
    if request.method == 'POST':
        form = GeocodeCompaniesForm(request.POST)
        if form.is_valid():
            from django.core.management import call_command
            mode = form.cleaned_data['mode']
            limit = form.cleaned_data.get('limit')
            include_contacts = form.cleaned_data.get('include_contacts')
            ...
            if include_contacts:
                args.append('--include-contacts')
            if dry_run:
                args.append('--dry-run')
            buffer = StringIO()
            try:
                call_command('geocode_pfmp_companies', *args, stdout=buffer)
                output = buffer.getvalue()
                messages.success(request, 'Géocodage PFMP terminé. Consulte le rapport ci-dessous.')
            except Exception as exc:
                output = buffer.getvalue() + '\nERREUR : ' + str(exc)
                messages.error(request, 'Le géocodage PFMP a rencontré une erreur. Consulte le rapport ci-dessous.')
    else:
        form = GeocodeCompaniesForm()

    return render(request, 'pfmp_manager/geocode_companies.html', {
        'form': form,
        'stats': _geocoding_stats(),
        'output': output,
        'user': user,
    })
