...
run_manage_env lp-core-app "LP_ADMIN_USER=$ADMIN_USER" "LP_ADMIN_PASS=$ADMIN_PASS" shell <<'PY'
import os
...
# Priorité au username si disponible, sinon code.
obj = None
if "username" in fields:
    obj = CoreUser.objects.filter(username=username).first()
if obj is None and "code" in fields:
    obj = CoreUser.objects.filter(code=username).first()
if obj is None:
    kwargs = {"username": username} if "username" in fields else {"code": username}
    obj = CoreUser(**kwargs)
...
for svc in "${LP_MODULE_SERVICES[@]}"; do
  ...
  run_manage_env "$svc" "DJANGO_ADMIN_USER=$DJANGO_USER" "DJANGO_ADMIN_PASS=$DJANGO_PASS" shell <<'PY' || warn "Admin Django non créé pour ce module"
import os
