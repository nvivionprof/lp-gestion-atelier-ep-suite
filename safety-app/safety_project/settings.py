...
DATA_DIR = Path(os.getenv('SAFETY_DATA_DIR', BASE_DIR / 'data'))
DATA_DIR.mkdir(parents=True, exist_ok=True)
SECRET_KEY = os.getenv('DJANGO_SECRET_KEY', 'dev-only-change-me-safety')
DEBUG = os.getenv('DJANGO_DEBUG', '1') == '1'
ALLOWED_HOSTS = os.getenv('DJANGO_ALLOWED_HOSTS', '*').split(',')
CSRF_TRUSTED_ORIGINS = [x for x in os.getenv('CSRF_TRUSTED_ORIGINS', '').split(',') if x]
INSTALLED_APPS = [
    'django.contrib.admin','django.contrib.auth','django.contrib.contenttypes','django.contrib.sessions','django.contrib.messages','django.contrib.staticfiles','safety_manager',
]
MIDDLEWARE = [
...
TEMPLATES = [{
    ...
    'OPTIONS': {'context_processors': [
        'django.template.context_processors.debug','django.template.context_processors.request','django.contrib.auth.context_processors.auth','django.contrib.messages.context_processors.messages','safety_manager.context_processors.safety_context',
    ]},
}]
...
DATABASES = {
    'default': {
        ...
        'NAME': os.getenv('DB_NAME', str(DATA_DIR / 'safety.sqlite3')),
...
SAFETY_APP_NAME=os.getenv('SAFETY_APP_NAME', 'Safety Manager')
SAFETY_VERSION=os.getenv('SAFETY_VERSION', 'Safety Manager — Bêta V0.2.0')

# Cookies isolés par application : évite les collisions entre modules servis sous le même domaine.
SESSION_COOKIE_NAME = os.getenv('SAFETY_SESSION_COOKIE_NAME', 'safety_sessionid')
CSRF_COOKIE_NAME = os.getenv('SAFETY_CSRF_COOKIE_NAME', 'safety_csrftoken')
SESSION_COOKIE_SAMESITE = os.getenv('SESSION_COOKIE_SAMESITE', 'Lax')
CSRF_COOKIE_SAMESITE = os.getenv('CSRF_COOKIE_SAMESITE', 'Lax')

SAFETY_RESET_PASSWORDS_ON_SYNC=os.getenv('SAFETY_RESET_PASSWORDS_ON_SYNC','0') == '1'
