...
DATA_DIR = Path(os.getenv('SYSTEM_MANAGER_DATA_DIR', BASE_DIR / 'data'))
DATA_DIR.mkdir(parents=True, exist_ok=True)
SECRET_KEY = os.getenv('DJANGO_SECRET_KEY', 'dev-only-change-me-system-manager')
...
INSTALLED_APPS = [
    ...
    'django.contrib.messages', 'django.contrib.staticfiles', 'system_manager',
]
MIDDLEWARE = [
...
TEMPLATES = [{
    ...
    'OPTIONS': {'context_processors': [
        ...
        'system_manager.context_processors.system_context',
    ]},
}]
...
DATABASES = {
    'default': {
        ...
        'NAME': os.getenv('DB_NAME', str(DATA_DIR / 'system-manager.sqlite3')),
...
PEDASHOP_PORT = os.getenv('PEDASHOP_PORT', '9003')
...
PEDASHOP_PUBLIC_URL = os.getenv('PEDASHOP_PUBLIC_URL', f'http://{SERVER_IP}:{PEDASHOP_PORT}').rstrip('/')
SYSTEM_MANAGER_PUBLIC_URL = os.getenv('SYSTEM_MANAGER_PUBLIC_URL', os.getenv('INVENTORY_PUBLIC_URL', f'http://{SERVER_IP}:{SYSTEM_MANAGER_PORT}')).rstrip('/')
TPMANAGER_PUBLIC_URL = os.getenv('TPMANAGER_PUBLIC_URL', f'http://{SERVER_IP}:{TPMANAGER_PORT}').rstrip('/')
PFMP_PUBLIC_URL = os.getenv('PFMP_PUBLIC_URL', f'http://{SERVER_IP}:9000/pfmp').rstrip('/')
LP_CORE_API_URL = os.getenv('LP_CORE_API_URL', 'http://lp-core-app:8000').rstrip('/')
LP_CORE_API_TOKEN = os.getenv('LP_CORE_API_TOKEN', '')
SYSTEM_MANAGER_APP_NAME = os.getenv('SYSTEM_MANAGER_APP_NAME', 'System Manager')
SYSTEM_MANAGER_VERSION = os.getenv('SYSTEM_MANAGER_VERSION', 'System Manager — Bêta V0.3.9 affichage dynamique / menu suite / checks')

# Cookies isolés par application : évite les collisions entre modules servis sous le même domaine.
SESSION_COOKIE_NAME = os.getenv('SYSTEM_MANAGER_SESSION_COOKIE_NAME', 'system_manager_sessionid')
CSRF_COOKIE_NAME = os.getenv('SYSTEM_MANAGER_CSRF_COOKIE_NAME', 'system_manager_csrftoken')
SESSION_COOKIE_SAMESITE = os.getenv('SESSION_COOKIE_SAMESITE', 'Lax')
CSRF_COOKIE_SAMESITE = os.getenv('CSRF_COOKIE_SAMESITE', 'Lax')

SYSTEM_MANAGER_RESET_PASSWORDS_ON_SYNC = os.getenv('SYSTEM_MANAGER_RESET_PASSWORDS_ON_SYNC', '0') == '1'
